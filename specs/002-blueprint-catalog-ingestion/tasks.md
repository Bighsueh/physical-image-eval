...
- [X] T009 [P] [US1] Build the **valid** synthetic fixture tree (8 region folders, 51 blueprint `.md` each with 整體資訊 + 4 `### N.` panels + AI prompt block, 51 placeholder `.png`, one `00_藍圖總索引與設計規範.md` with the 134-row matrix) in `backend/tests/fixtures/source/valid/`.
...
- [X] T011 [P] [US1] Unit test the index parser (00 index → 134 `ParsedDiagnosis` with `matrixNo`, `mappingKind` derived by Y-series rule, `mappedBlueprint`; 6 referral) in `backend/tests/unit/ingestion/index-parser.test.ts`.
...
- [X] T014 [US1] Integration test: full `ingest` against `fixtures/source/valid/` → 51 `Blueprint`, region counts S4..Y12, exactly 4 `Panel` per blueprint, non-empty metadata, 134 `Diagnosis` (128 mapped+template / 6 referral), high-risk set, exit 0 + success report in `backend/tests/integration/ingestion/ingest-success.test.ts`.
- [X] T015 [P] [US1] Integration test the read API: `GET /api/regions` (8, ordered, `blueprintCount`), `GET /api/blueprints` (51 + `region`/`highRisk` filters), `GET /api/blueprints/:id` (4 ordered panels incl. `visualDescription`, `aiPrompt` **absent**), `GET /api/diagnoses` (134 + reconciliation `meta`), envelope shape, `401 AUTH_REQUIRED` without session in `backend/tests/integration/catalog-api/catalog-read.test.ts`.
...
- [X] T020 [P] [US1] Implement `backend/src/ingestion/parser/index-parser.ts` — 00 index tables → `ParsedDiagnosis[]`; Y-series ⇒ `TEMPLATE`, non-Y ⇒ `MAPPED`, referral table ⇒ `REFERRAL` (FR-008/D2).
- [X] T021 [US1] Implement `backend/src/ingestion/validation/invariants.ts` — all fail-fast checks over the parsed set: 51 count (FR-002), region counts (FR-003), exactly 4 panels {1,2,3,4} (FR-004), single image + no bidirectional orphan (FR-005), non-empty indications/frequency/gentleReminder (FR-006), non-empty every `actionDescription` (FR-007), 134 reconciliation = 128(MAPPED+TEMPLATE)+6 REFERRAL (FR-008), every non-referral `mappedBlueprint` resolves to an existing blueprint (FR-009), high-risk set == `HIGH_RISK_BLUEPRINT_IDS` exactly (FR-010), legal+unique IDs (FR-022), per-blueprint `涵蓋診斷` subset cross-check (D2) — returns `ReportError[]` (depends on T018, T020 + constants/types).
