...
背景：團隊為「物理治療衛教 AI 助理」產出了一系列**四格漫畫（2×2 四宮格）運動衛教圖**，由 AI 生成，涵蓋肩、頸、肘腕手、脊椎軀幹、骨盆髖、膝、小腿足踝，以及全身性運動處方（骨鬆、防跌、心肺、代謝等）。這些圖在上線給病人看之前，**需要醫師逐張把關臨床正確性與安全性**。本工具就是承載這個審查流程的介面。
...
AIGC 圖像來源目錄（**唯讀，不得修改或寫入**）以環境變數設定，不寫死在 repo：
...
- `_產圖/<區域資料夾>/<編號_名稱>.png` — 衛教圖 PNG，依 8 個解剖區域分資料夾。
- `<區域資料夾>/<編號_名稱>.md` — 每張圖對應的完整圖文企劃（整體資訊、4 宮格分鏡、AI 產圖 prompt）。
- `00_藍圖總索引與設計規範.md` — 總索引、診斷 → 藍圖對照表（匯入時藍圖集合與診斷編號以此為準）、共用視覺設計規範。
...
- **生產**：以 **Cloudflared** 託管；正式網域不寫進 repo，由部署環境的 `COOKIE_DOMAIN` 設定。
