...
describe('progress / next / isolation (US3/US4/US7)', () => {
  ...
  it('GET /progress: empty reviewer → 0/51; counts + perRegion + filterable index', async () => {
    const empty = await r.agent.get('/api/reviews/progress');
    expect(empty.body.data).toMatchObject({ submitted: 0, draft: 0, notStarted: 51, total: 51 });
    expect(empty.body.data.perRegion).toHaveLength(8);
    expect(empty.body.data.perRegion[0]).toMatchObject({ regionCode: 'S', total: 4 });
    expect(empty.body.data.index).toHaveLength(51);
  ...
  it('submitting all 51 → /next completed, progress 51/51 (SC-008)', async () => {
    ...
    expect(next.body.data).toMatchObject({ next: null, completed: true, submitted: 51, total: 51 });
    expect((await r.agent.get('/api/reviews/progress')).body.data.submitted).toBe(51);
  });
});
